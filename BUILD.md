...
## ⚠️ اقرا ده الأول — مرة واحدة بس

الملف اللي بعتهولك فيه `app.json` **من غير رقم مشروعك على EAS** — لأن الرقم
ده بتاعك انت، مش بتاع الكود. يعني كل مرة تفك الملف فوق الفولدر القديم،
بيمسح الربط بين الفولدر وحسابك.

**التصليح — مرة واحدة وخلاص:**

```bash
npx eas-cli init
```

هيسألك تختار مشروع. **اختار `masari` الموجود.**

❌ **متختارش «create a new project»** — المشروع الجديد بيبدأ فاضي من غير
نسخ، فمش هيقدر يحدّث أي حاجة على تليفونك.

بعد كده ابعتلي الرقم اللي هيظهر (شكله كده `xxxxxxxx-xxxx-...`) وأنا أحطه
في الملف، وميحصلش تاني.
